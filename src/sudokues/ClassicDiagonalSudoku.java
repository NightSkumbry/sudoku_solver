...
import grids.ClassicDiagonalGrid;
...
public class ClassicDiagonalSudoku extends AbstractSudoku<ClassicDiagonalGrid, ClassicCell> {
    public ClassicDiagonalSudoku(List<Integer> grid) {
        super(new ClassicDiagonalGrid(grid));
    }
    ...
    private boolean computeInHGroups(int groupSize) {
        ...
        RemoveInHGroupsOperation<ClassicDiagonalGrid, ClassicCell> removeInHGroupsOperation = new RemoveInHGroupsOperation<>(history.getNextOperationID());
        ...
        // check main diagonal
        {
            List<ClassicCell> mainDiagonalCells = currentGrid.getMainDiagonal();
            Set<Integer> mainDiagonalPossibleValues = new HashSet<>();
            mainDiagonalCells.forEach(mainDiagonalCell -> {
                if (!mainDiagonalCell.isSet()) mainDiagonalPossibleValues.addAll(mainDiagonalCell.getPossibleValues());
            });
            if (mainDiagonalPossibleValues.size() > groupSize) {
                for (Set<Integer> valueCombination : Sets.combinations(mainDiagonalPossibleValues, groupSize)) {
                    List<Integer> possibleCellIndices = new ArrayList<>();
                    for (int mainDiagonalCellIndex = 0; mainDiagonalCellIndex < 9; mainDiagonalCellIndex++) {
                        for (int v : valueCombination) {
                            if (mainDiagonalCells.get(mainDiagonalCellIndex).getPossibleValues().contains(v)) {
                                possibleCellIndices.add(mainDiagonalCellIndex);
                                break;
                            }
                        }
                    }
                    ...
                    if (possibleCellIndices.size() <= valueCombination.size()) {
                        for (int mainDiagonalCellIndex = 0; mainDiagonalCellIndex < 9; mainDiagonalCellIndex++) {
                            ClassicCell cell = mainDiagonalCells.get(mainDiagonalCellIndex);
                            if (cell.isSet() || !possibleCellIndices.contains(mainDiagonalCellIndex)) continue;
                            for (int i = 0; i < 9; i++) {
                                if (valueCombination.contains(i)) continue;
                                int fullCellIndex = currentGrid.getIndexOnMainDiagonal(mainDiagonalCellIndex);
                                boolean r = bufferGrid.get(fullCellIndex).removePossibleValue(i);
                                if (r) {
                                    List<Integer> groupIndices = new ArrayList<>();
                                    List<Integer> nonGroupIndices = new ArrayList<>();
                                    List<Integer> toPurple = new ArrayList<>();
                                    for (int rcl = 0; rcl < 9; rcl++) {
                                        if (possibleCellIndices.contains(rcl)) groupIndices.add(currentGrid.getIndexOnMainDiagonal(rcl));
                                        else if (!mainDiagonalCells.get(rcl).isSet()) nonGroupIndices.add(currentGrid.getIndexOnMainDiagonal(rcl));
                                        else toPurple.add(currentGrid.getIndexOnMainDiagonal(rcl));
                                    }
                                    removeInHGroupsOperation.addStep(new RemovePossibleHGroupStep<>(fullCellIndex, i, groupIndices, nonGroupIndices, toPurple));
                                }
                            }
        ...
        // check side diagonal
        {
            List<ClassicCell> sideDiagonalCells = currentGrid.getSideDiagonal();
            Set<Integer> sideDiagonalPossibleValues = new HashSet<>();
            sideDiagonalCells.forEach(sideDiagonalCell -> {
                if (!sideDiagonalCell.isSet()) sideDiagonalPossibleValues.addAll(sideDiagonalCell.getPossibleValues());
            });
            if (sideDiagonalPossibleValues.size() > groupSize) {
                for (Set<Integer> valueCombination : Sets.combinations(sideDiagonalPossibleValues, groupSize)) {
                    List<Integer> possibleCellIndices = new ArrayList<>();
                    for (int sideDiagonalCellIndex = 0; sideDiagonalCellIndex < 9; sideDiagonalCellIndex++) {
                        for (int v : valueCombination) {
                            if (sideDiagonalCells.get(sideDiagonalCellIndex).getPossibleValues().contains(v)) {
                                possibleCellIndices.add(sideDiagonalCellIndex);
                                break;
                            }
                        }
                    }
                    ...
                    if (possibleCellIndices.size() <= valueCombination.size()) {
                        for (int sideDiagonalCellIndex = 0; sideDiagonalCellIndex < 9; sideDiagonalCellIndex++) {
                            ClassicCell cell = sideDiagonalCells.get(sideDiagonalCellIndex);
                            if (cell.isSet() || !possibleCellIndices.contains(sideDiagonalCellIndex)) continue;
                            for (int i = 0; i < 9; i++) {
                                if (valueCombination.contains(i)) continue;
                                int fullCellIndex = currentGrid.getIndexOnSideDiagonal(sideDiagonalCellIndex);
                                boolean r = bufferGrid.get(fullCellIndex).removePossibleValue(i);
                                if (r) {
                                    List<Integer> groupIndices = new ArrayList<>();
                                    List<Integer> nonGroupIndices = new ArrayList<>();
                                    List<Integer> toPurple = new ArrayList<>();
                                    for (int rcl = 0; rcl < 9; rcl++) {
                                        if (possibleCellIndices.contains(rcl)) groupIndices.add(currentGrid.getIndexOnSideDiagonal(rcl));
                                        else if (!sideDiagonalCells.get(rcl).isSet()) nonGroupIndices.add(currentGrid.getIndexOnSideDiagonal(rcl));
                                        else toPurple.add(currentGrid.getIndexOnSideDiagonal(rcl));
                                    }
                                    removeInHGroupsOperation.addStep(new RemovePossibleHGroupStep<>(fullCellIndex, i, groupIndices, nonGroupIndices, toPurple));
                                }
                            }
    ...
    private boolean computeOutsideHGroups(int groupSize) {
        ...
        RemoveOutsideHGroupsOperation<ClassicDiagonalGrid, ClassicCell> removeOutsideHGroupsOperation = new RemoveOutsideHGroupsOperation<>(history.getNextOperationID());
        ...
        {
            List<ClassicCell> mainDiagonalCells = currentGrid.getMainDiagonal();
            Set<Integer> mainDiagonalEmptyCells = new HashSet<>();
            for (int i = 0; i < 9; i++) {
                if (!mainDiagonalCells.get(i).isSet()) mainDiagonalEmptyCells.add(i);
            }
            if (mainDiagonalEmptyCells.size() > groupSize) {
                for (Set<Integer> cellCombination : Sets.combinations(mainDiagonalEmptyCells, groupSize)) {
                    Set<Integer> possibleValues = new HashSet<>();
                    for (int mainDiagonalCellIndex : cellCombination) {
                        possibleValues.addAll(mainDiagonalCells.get(mainDiagonalCellIndex).getPossibleValues());
                    }
                    
                    if (possibleValues.size() <= cellCombination.size()) {
                        for (int mainDiagonalCellIndex = 0; mainDiagonalCellIndex < 9; mainDiagonalCellIndex++) {
                            ClassicCell cell = mainDiagonalCells.get(mainDiagonalCellIndex);
                            if (cell.isSet() || cellCombination.contains(mainDiagonalCellIndex)) continue;
                            for (int i = 0; i < 9; i++) {
                                if (!possibleValues.contains(i)) continue;
                                int fullCellIndex = currentGrid.getIndexOnMainDiagonal(mainDiagonalCellIndex);
                                boolean r = bufferGrid.get(fullCellIndex).removePossibleValue(i);
                                if (r) {
                                    ...
                                    List<Integer> toPurple = new ArrayList<>();
                                    for (int rcl = 0; rcl < 9; rcl++) {
                                        if (cellCombination.contains(rcl)) groupIndices.add(currentGrid.getIndexOnMainDiagonal(rcl));
                                        else if (!mainDiagonalCells.get(rcl).isSet()) nonGroupIndices.add(currentGrid.getIndexOnMainDiagonal(rcl));
                                        else toPurple.add(currentGrid.getIndexOnMainDiagonal(rcl));
                                    }
                                    removeOutsideHGroupsOperation.addStep(new RemovePossibleHGroupStep<>(fullCellIndex, i, groupIndices, nonGroupIndices, toPurple));
        ...
        {
            List<ClassicCell> sideDiagonalCells = currentGrid.getSideDiagonal();
            Set<Integer> sideDiagonalEmptyCells = new HashSet<>();
            for (int i = 0; i < 9; i++) {
                if (!sideDiagonalCells.get(i).isSet()) sideDiagonalEmptyCells.add(i);
            }
            if (sideDiagonalEmptyCells.size() > groupSize) {
                for (Set<Integer> cellCombination : Sets.combinations(sideDiagonalEmptyCells, groupSize)) {
                    Set<Integer> possibleValues = new HashSet<>();
                    for (int sideDiagonalCellIndex : cellCombination) {
                        possibleValues.addAll(sideDiagonalCells.get(sideDiagonalCellIndex).getPossibleValues());
                    }
                    
                    if (possibleValues.size() <= cellCombination.size()) {
                        for (int sideDiagonalCellIndex = 0; sideDiagonalCellIndex < 9; sideDiagonalCellIndex++) {
                            ClassicCell cell = sideDiagonalCells.get(sideDiagonalCellIndex);
                            if (cell.isSet() || cellCombination.contains(sideDiagonalCellIndex)) continue;
                            for (int i = 0; i < 9; i++) {
                                if (!possibleValues.contains(i)) continue;
                                int fullCellIndex = currentGrid.getIndexOnSideDiagonal(sideDiagonalCellIndex);
                                boolean r = bufferGrid.get(fullCellIndex).removePossibleValue(i);
                                if (r) {
                                    ...
                                    List<Integer> toPurple = new ArrayList<>();
                                    for (int rcl = 0; rcl < 9; rcl++) {
                                        if (cellCombination.contains(rcl)) groupIndices.add(currentGrid.getIndexOnSideDiagonal(rcl));
                                        else if (!sideDiagonalCells.get(rcl).isSet()) nonGroupIndices.add(currentGrid.getIndexOnSideDiagonal(rcl));
                                        else toPurple.add(currentGrid.getIndexOnSideDiagonal(rcl));
                                    }
                                    removeOutsideHGroupsOperation.addStep(new RemovePossibleHGroupStep<>(fullCellIndex, i, groupIndices, nonGroupIndices, toPurple));
    ...
    private boolean computePointingValues() {
        ...
        RemovePointingOperation<ClassicDiagonalGrid, ClassicCell> removePointingOperation = new RemovePointingOperation<>(history.getNextOperationID());
        ...
        // check single main diagonal cell possible in box
        for (int box = 0; box < 9; box+=4) {
            List<ClassicCell> boxCells = currentGrid.getBox(box);
            for (int n = 0; n < 9; n++) {
                boolean isOnMainDiagonal = false;
                boolean isNotOnMainDiagonal = false;
                for (int k = 0; k < 9; k++) {
                    ClassicCell cell = boxCells.get(k);
                    if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                        if (currentGrid.isOnMainDiagonal(currentGrid.getIndexByBoxIndex(box, k))) isOnMainDiagonal = true;
                        else isNotOnMainDiagonal = true;
                    }
                }
                if (isOnMainDiagonal && !isNotOnMainDiagonal) {
                    List<Integer> pointingCellIndices = new ArrayList<>();
                    List<Integer> reasonCellIndices = new ArrayList<>();
                    List<Integer> toPurpleCellIndices = new ArrayList<>();
                    for (int k = 0; k < 9; k++) {
                        ClassicCell cell = boxCells.get(k);
                        if (!cell.isSet() && (!currentGrid.isOnMainDiagonal(currentGrid.getIndexByBoxIndex(box, k)) || !cell.getPossibleValues().contains(n))) {
                            reasonCellIndices.add(currentGrid.getIndexByBoxIndex(box, k));
                        }
                        else if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                            pointingCellIndices.add(currentGrid.getIndexByBoxIndex(box, k));
                    ...
                    for (int mainDiagonalCellIndex=0; mainDiagonalCellIndex < 9; mainDiagonalCellIndex++) {
                        int cellIndex = currentGrid.getIndexOnMainDiagonal(mainDiagonalCellIndex);
                        if (!pointingCellIndices.contains(cellIndex) && !currentGrid.get(cellIndex).isSet()) {
                            boolean r = bufferGrid.get(cellIndex).removePossibleValue(n);
                            if (r) {
                                removePointingOperation.addStep(new RemovePossibleInPointingStep<>(cellIndex, n, reasonCellIndices, pointingCellIndices, toPurpleCellIndices));
        ...
        // check single side diagonal cell possible in box
        for (int box = 0; box < 9; box+=4) {
            List<ClassicCell> boxCells = currentGrid.getBox(box);
            for (int n = 0; n < 9; n++) {
                boolean isOnSideDiagonal = false;
                boolean isNotOnSideDiagonal = false;
                for (int k = 0; k < 9; k++) {
                    ClassicCell cell = boxCells.get(k);
                    if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                        if (currentGrid.isOnSideDiagonal(currentGrid.getIndexByBoxIndex(box, k))) isOnSideDiagonal = true;
                        else isNotOnSideDiagonal = true;
                    }
                }
                if (isOnSideDiagonal && !isNotOnSideDiagonal) {
                    List<Integer> pointingCellIndices = new ArrayList<>();
                    List<Integer> reasonCellIndices = new ArrayList<>();
                    List<Integer> toPurpleCellIndices = new ArrayList<>();
                    for (int k = 0; k < 9; k++) {
                        ClassicCell cell = boxCells.get(k);
                        if (!cell.isSet() && (!currentGrid.isOnSideDiagonal(currentGrid.getIndexByBoxIndex(box, k)) || !cell.getPossibleValues().contains(n))) {
                            reasonCellIndices.add(currentGrid.getIndexByBoxIndex(box, k));
                        }
                        else if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                            pointingCellIndices.add(currentGrid.getIndexByBoxIndex(box, k));
                    ...
                    for (int sideDiagonalCellIndex=0; sideDiagonalCellIndex < 9; sideDiagonalCellIndex++) {
                        int cellIndex = currentGrid.getIndexOnSideDiagonal(sideDiagonalCellIndex);
                        if (!pointingCellIndices.contains(cellIndex) && !currentGrid.get(cellIndex).isSet()) {
                            boolean r = bufferGrid.get(cellIndex).removePossibleValue(n);
                            if (r) {
                                removePointingOperation.addStep(new RemovePossibleInPointingStep<>(cellIndex, n, reasonCellIndices, pointingCellIndices, toPurpleCellIndices));
        ...
        // check single box cell possible in main diagonal
        {
            List<ClassicCell> mainDiagonalCells = currentGrid.getMainDiagonal();
            for (int n = 0; n < 9; n++) {
                Set<Integer> possibleBoxPlaces = new HashSet<>();
                for (int k = 0; k < 9; k++) {
                    ClassicCell cell = mainDiagonalCells.get(k);
                    if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                        possibleBoxPlaces.add(currentGrid.getBoxIndex(currentGrid.getIndexOnMainDiagonal(k)));
                    }
                }
                if (possibleBoxPlaces.size() == 1) {
                    int boxIndex = possibleBoxPlaces.iterator().next();
                    ...
                    for (int k = 0; k < 9; k++) {
                        ClassicCell cell = mainDiagonalCells.get(k);
                        if (!cell.isSet() && (currentGrid.getBoxIndex(currentGrid.getIndexOnMainDiagonal(k)) != boxIndex || !cell.getPossibleValues().contains(n))) {
                            reasonCellIndices.add(currentGrid.getIndexOnMainDiagonal(k));
                        }
                        else if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                            pointingCellIndices.add(currentGrid.getIndexOnMainDiagonal(k));
                        }
                        else toPurpleCellIndices.add(currentGrid.getIndexOnMainDiagonal(k));
                    }
        ...
        // check single box cell possible in side diagonal
        {
            List<ClassicCell> sideDiagonalCells = currentGrid.getSideDiagonal();
            for (int n = 0; n < 9; n++) {
                Set<Integer> possibleBoxPlaces = new HashSet<>();
                for (int k = 0; k < 9; k++) {
                    ClassicCell cell = sideDiagonalCells.get(k);
                    if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                        possibleBoxPlaces.add(currentGrid.getBoxIndex(currentGrid.getIndexOnSideDiagonal(k)));
                    }
                }
                if (possibleBoxPlaces.size() == 1) {
                    int boxIndex = possibleBoxPlaces.iterator().next();
                    ...
                    for (int k = 0; k < 9; k++) {
                        ClassicCell cell = sideDiagonalCells.get(k);
                        if (!cell.isSet() && (currentGrid.getBoxIndex(currentGrid.getIndexOnSideDiagonal(k)) != boxIndex || !cell.getPossibleValues().contains(n))) {
                            reasonCellIndices.add(currentGrid.getIndexOnSideDiagonal(k));
                        }
                        else if (!cell.isSet() && cell.getPossibleValues().contains(n)) {
                            pointingCellIndices.add(currentGrid.getIndexOnSideDiagonal(k));
                        }
                        else toPurpleCellIndices.add(currentGrid.getIndexOnSideDiagonal(k));
                    }
    ...
    private boolean placeSingles() {
        ...
        PlaceSinglesOperation<ClassicDiagonalGrid, ClassicCell> placeSinglesOperation = new PlaceSinglesOperation<>(history.getNextOperationID());    
        ...
        {
            List<ClassicCell> mainDiagonalCells = currentGrid.getMainDiagonal();
            List<ClassicCell> bufferMainDiagonalCells = bufferGrid.getMainDiagonal();
            for (int n = 0; n < 9; n++) {
                Set<Integer> possiblePlaces = new HashSet<>();
                for (int k = 0; k < 9; k++) {
                    ClassicCell cell = mainDiagonalCells.get(k);
                ...
                if (possiblePlaces.size() == 1) {
                    int cellIndex = possiblePlaces.iterator().next();
                    bufferMainDiagonalCells.get(cellIndex).setValue(n);
                    List<Integer> emptyIndices = new ArrayList<>();
                    for (int i = 0; i < 9; i++) {
                        ClassicCell c = bufferMainDiagonalCells.get(i);
                        if (!c.isSet()) {
                            emptyIndices.add(currentGrid.getIndexOnMainDiagonal(i));
                        }
                    }
                    placeSinglesOperation.addStep(new SingleInStep<>(currentGrid.getIndexOnMainDiagonal(cellIndex), n, emptyIndices));
                }
            }
        }
        ...
        {
            List<ClassicCell> sideDiagonalCells = currentGrid.getSideDiagonal();
            List<ClassicCell> bufferSideDiagonalCells = bufferGrid.getSideDiagonal();
            for (int n = 0; n < 9; n++) {
                Set<Integer> possiblePlaces = new HashSet<>();
                for (int k = 0; k < 9; k++) {
                    ClassicCell cell = sideDiagonalCells.get(k);
                ...
                if (possiblePlaces.size() == 1) {
                    int cellIndex = possiblePlaces.iterator().next();
                    bufferSideDiagonalCells.get(cellIndex).setValue(n);
                    List<Integer> emptyIndices = new ArrayList<>();
                    for (int i = 0; i < 9; i++) {
                        ClassicCell c = bufferSideDiagonalCells.get(i);
                        if (!c.isSet()) {
                            emptyIndices.add(currentGrid.getIndexOnSideDiagonal(i));
                        }
                    }
                    placeSinglesOperation.addStep(new SingleInStep<>(currentGrid.getIndexOnSideDiagonal(cellIndex), n, emptyIndices));
                }
            }
        }
    ...
    private boolean placeObvious() {
        ...
        PlaceObviousOperation<ClassicDiagonalGrid, ClassicCell> placeObviousOperation = new PlaceObviousOperation<>(history.getNextOperationID());
    ...
    private boolean computePossibles() {
        ...
        ComputePossiblesOperation<ClassicDiagonalGrid, ClassicCell> computePossiblesOperation = new ComputePossiblesOperation<>(history.getNextOperationID());
        ...
        // check main diagonal
        {
            List<ClassicCell> mainDiagonal = currentGrid.getMainDiagonal();
            Integer[] values = new Integer[9];
            Set<Integer> occupied = new HashSet<>();
            for (int j = 0; j < 9; j++) {
                if (mainDiagonal.get(j).isSet()) {
                    values[mainDiagonal.get(j).getValue()] = j;
                    occupied.add(mainDiagonal.get(j).getValue());
                }
            }
            ...
            for (int j = 0; j < 9; j++) {
                if (!mainDiagonal.get(j).isSet()) {
                    for (Integer v: occupied) {
                        boolean r = mainDiagonal.get(j).removePossibleValue(v);
                        if (r) {
                            computePossiblesOperation.addStep(new RemovePossibleStep<>(currentGrid.getIndexOnMainDiagonal(j), v, currentGrid.getIndexOnMainDiagonal(values[v])));
                        }
                    }
                }
            }
        ...
        // check side diagonal
        {
            List<ClassicCell> sideDiagonal = currentGrid.getSideDiagonal();
            Integer[] values = new Integer[9];
            Set<Integer> occupied = new HashSet<>();
            for (int j = 0; j < 9; j++) {
                if (sideDiagonal.get(j).isSet()) {
                    values[sideDiagonal.get(j).getValue()] = j;
                    occupied.add(sideDiagonal.get(j).getValue());
                }
            }
            ...
            for (int j = 0; j < 9; j++) {
                if (!sideDiagonal.get(j).isSet()) {
                    for (Integer v: occupied) {
                        boolean r = sideDiagonal.get(j).removePossibleValue(v);
                        if (r) {
                            computePossiblesOperation.addStep(new RemovePossibleStep<>(currentGrid.getIndexOnSideDiagonal(j), v, currentGrid.getIndexOnSideDiagonal(values[v])));
                        }
                    }
                }
            }
